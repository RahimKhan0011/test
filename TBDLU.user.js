...
(function() {
    ...
    const MAX_IMG_SIZE = 256 * 1024;
    const OMDB_API_KEYS = ['9990cb2f', '79ee18d', 'bc90b6c5'];
    const CATEGORY_MOVIE = '3';
    const CATEGORY_TV = '5';
    const CATEGORY_DOCUMENTARY = '9';
    const CATEGORY_ANIME = '28';
    ...
    let currentOmdbApiKeyIndex = 0;
    let imdbCategoryOverrideTimer = null;
    let lastImdbLookupId = '';
        ...
        const open = () => {
            ...
            requestAnimationFrame(() => {
                ...
                setTimeout(() => {
                    ...
                    if(val) {
                        const baseTitle = val
                            .replace(/\s*(?:\(\s*aka\s*\)|\[\s*aka\s*\]|\{\s*aka\s*\}|\baka\b)\s*[:\-–—]?\s*.*$/i, '')
                            .trim();
                        const clean = baseTitle.replace(/(\.|-|\s)(2160p|1080p|720p|S\d+|E\d+|WEB|Bluray).*/i, '').replace(/[.-]/g, ' ').trim();
    ...
    function normalizeTitleForAlias(title) {
    ...
    function hasAliasAlready(normCurrent, normSelected) {
        const aliasPattern = new RegExp(`\\baka\\s+${escapeRegExp(normSelected)}(?:\\s|$)`);
        return aliasPattern.test(normCurrent);
    }

    function startsWithSelectedWord(normCurrent, normSelected) {
        const nextChar = normCurrent[normSelected.length];
        return normCurrent.startsWith(normSelected) && (!nextChar || /\W/.test(nextChar));
    }

    function asStringCandidates(value) {
        if (!value) return [];
    ...
    function getImdbAliasCandidates(item) {
        const rawCandidates = [
            ...(item ? asStringCandidates(item.aka) : []),
            ...(item ? asStringCandidates(item.akas) : []),
            ...(item ? asStringCandidates(item.originalTitle) : []),
            ...(item ? asStringCandidates(item.original_title) : [])
        ];
        const unique = [];
        const seen = new Set();
        rawCandidates.forEach((candidate) => {
            const normalized = normalizeTitleForAlias(candidate);
    ...
    function insertAliasIntoTitle(title, alias) {
        const markerPattern = /\b(S\d{1,2}E\d{1,2}|S\d{1,2}|E\d{1,2}|(?:480|540|720|900|1080|2160|4320)p|WEB[- ]DL|WEBRip|BluRay|BRRip|BDRip|HDRip|NF|AMZN|UHD|REMUX|DVDRip)\b/i;
        const match = title.match(markerPattern);
        if (match && match.index > 0) {
            const before = title.slice(0, match.index).trimEnd();
            const after = title.slice(match.index).trimStart();
            return `${before} aka ${alias} ${after}`.replace(/\s{2,}/g, ' ');
        }
        return `${title} aka ${alias}`;
    }

    function maybeAppendAliasTitle(selectedTitle) {
        if (!selectedTitle) return false;
        const titleInput = document.getElementById('torrent_name');
        if (!titleInput) return false;

        const currentTitle = titleInput.value.trim();
        if (!currentTitle) return false;
        if (/\baka\b/i.test(currentTitle)) return false;

        const normCurrent = normalizeTitleForAlias(currentTitle);
        const normSelected = normalizeTitleForAlias(selectedTitle);

        if (!normSelected) return false;

        const alreadyHasAlias = hasAliasAlready(normCurrent, normSelected);
        const sameTitle = normCurrent === normSelected;
        const startsWithSelected = startsWithSelectedWord(normCurrent, normSelected);
        if (alreadyHasAlias || sameTitle || startsWithSelected) return false;

        titleInput.value = insertAliasIntoTitle(currentTitle, selectedTitle);
    ...
    function selectImdb(item) {
        ...
        const aliases = getImdbAliasCandidates(item);
        let aliasAdded = false;
        for (const alias of aliases) {
            if (maybeAppendAliasTitle(alias)) {
                aliasAdded = true;
                break;
            }
        }
        if (!aliasAdded) maybeAppendAliasTitle(item.l);
    ...
    function getNextOmdbApiKey() {
        const key = OMDB_API_KEYS[currentOmdbApiKeyIndex];
        currentOmdbApiKeyIndex = (currentOmdbApiKeyIndex + 1) % OMDB_API_KEYS.length;
        return key;
    }
    ...
    function setCategoryValue(value) {
        ...
        if (categorySelect.value === String(value)) return;
        categorySelect.value = String(value);
        categorySelect.dispatchEvent(new Event('change', { bubbles: true }));
    }

    function cleanTitleForOmdbSearch(title) {
        return (title || '')
            .replace(/\s*\baka\b.*$/i, ' ')
            .replace(/[._]/g, ' ')
            .replace(/(\sS\d+E\d+|\sS\d+|\sE\d+).*$/i, ' ')
            .replace(/\b(2160p|1080p|720p|480p|UHD|WEB[- ]DL|WEBRip|BluRay|BRRip|BDRip|HDRip|DVDRip|REMUX|x264|x265|HEVC|AAC|DD\+?\d\.\d|DTS)\b/gi, ' ')
            .replace(/\s+/g, ' ')
            .trim();
    }

    function searchOmdbByTitle(title, callback, attempt = 0) {
        const apiKey = getNextOmdbApiKey();
        ...
        GM_xmlhttpRequest({
            ...
            onload: (res) => {
                if (res.status !== 200) {
                    if (attempt < OMDB_API_KEYS.length - 1) return searchOmdbByTitle(title, callback, attempt + 1);
                ...
                const hasSearchResults = Array.isArray(data?.Search) && data.Search.length > 0;
                if (!isValidResponse || !hasSearchResults) {
                    if (attempt < OMDB_API_KEYS.length - 1) return searchOmdbByTitle(title, callback, attempt + 1);
                    return callback(null);
                }
                let primaryResult = null;
                for (const result of data.Search) {
                    if (!result || !result.imdbID) continue;
    ...
    function applyImdbFieldFromSearchResult(match) {
        if (!match || !match.imdbID) return;
        const imdbField = document.getElementById('imdb_url');
        if (!imdbField || imdbField.value.trim()) return;
        imdbField.value = `https://www.imdb.com/title/${match.imdbID}/`;
        imdbField.dispatchEvent(new Event('input', { bubbles: true }));
    }

    function applyCategoryFromTitleSearch(title, fallbackCategory) {
        const cleanedTitle = cleanTitleForOmdbSearch(title);
        if (!cleanedTitle) {
            if (fallbackCategory) setCategoryValue(String(fallbackCategory));
            applyImdbCategoryOverride();
            return;
        }
        searchOmdbByTitle(cleanedTitle, (match) => {
            let categoryAppliedFromSearch = false;
            if (match) {
                if (match.Type === 'series') {
                    setCategoryValue(CATEGORY_TV);
                    categoryAppliedFromSearch = true;
                } else if (match.Type === 'movie') {
                    setCategoryValue(CATEGORY_MOVIE);
                    categoryAppliedFromSearch = true;
                }
            }
            if (!categoryAppliedFromSearch && fallbackCategory) setCategoryValue(String(fallbackCategory));
            applyImdbCategoryOverride();
        });
    }

    function applyImdbCategoryOverride(attempt = 0) {
        const imdbField = document.getElementById('imdb_url');
        const categorySelect = document.getElementById('filter');
        if (!imdbField || !categorySelect) return;

        const imdbMatch = imdbField.value.trim().match(/tt\d+/i);
        if (!imdbMatch) return;

        const imdbID = imdbMatch[0].toLowerCase();
        if (imdbID === lastImdbLookupId && (categorySelect.value === CATEGORY_DOCUMENTARY || categorySelect.value === CATEGORY_ANIME)) {
            return;
        }
        const apiKey = getNextOmdbApiKey();
        ...
        GM_xmlhttpRequest({
            ...
            onload: (res) => {
                if (res.status !== 200) {
                    if (attempt < OMDB_API_KEYS.length - 1) applyImdbCategoryOverride(attempt + 1);
                    return;
                }
                let data;
                try {
                ...
                if (genre.includes('Documentary') && categorySelect.value !== CATEGORY_DOCUMENTARY) {
                    categorySelect.value = CATEGORY_DOCUMENTARY;
                    categorySelect.dispatchEvent(new Event('change', { bubbles: true }));
                    lastImdbLookupId = imdbID;
                    return;
                }

                if (genre.includes('Animation') && language.includes('Japanese') && categorySelect.value !== CATEGORY_ANIME) {
                    categorySelect.value = CATEGORY_ANIME;
                    categorySelect.dispatchEvent(new Event('change', { bubbles: true }));
                    lastImdbLookupId = imdbID;
                }
            },
    ...
    function fillForm(data) {
        ...
        if (langSelect && data.language) {
            langSelect.value = String(data.language);
            langSelect.dispatchEvent(new Event('change', { bubbles: true }));
        }

        if (data.category) setCategoryValue(String(data.category));
        applyImdbCategoryOverride();
    ...
    buildUI();
    injectImdbSearch();
    const imdbInput = document.getElementById('imdb_url');
    if (imdbInput) {
        imdbInput.addEventListener('input', () => {
            if (imdbCategoryOverrideTimer) clearTimeout(imdbCategoryOverrideTimer);
            imdbCategoryOverrideTimer = setTimeout(() => applyImdbCategoryOverride(), 400);
        });
    }
